...
import { clampLabel, formatCompact, formatDate, formatPercent } from './format'
...
export function CatalogMarketCard({
  ...
  const showListingChip = !listed || market.pendleStatus === 'inactive'
  const showLifecycleChip = lifecycle !== 'Live'
  const showMetadataChip = metadataIncomplete && lifecycle !== 'Unknown'

  return (
    <Link
      to={marketPath(market.address, market.chainId)}
      className="group flex flex-col rounded-lg border border-hairline bg-surface p-4 text-fg no-underline transition duration-200 hover:-translate-y-0.5 hover:border-hairline-strong hover:shadow-[var(--op-shadow)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent motion-reduce:transform-none motion-reduce:transition-none"
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className="inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.08em]"
          title={chainName}
          style={{
      ...
      {(showListingChip || showLifecycleChip || showMetadataChip) && (
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          {showListingChip && (
            ...
            >
              {community ? 'Community' : listingLabel}
            </span>
          )}
          {showLifecycleChip && (
            <span
          ...
          {showMetadataChip && (
            <span className="rounded-full border border-hairline bg-surface-2 px-2 py-0.5 text-[10px] font-medium text-faint">
              Partial data
              <span className="sr-only"> — some data is missing; the market page loads it live</span>
            </span>
          )}
        </div>
      )}

      <div className="mt-3.5 grid grid-cols-3 gap-2 border-t border-hairline pt-3">
        <div>
          <p className="text-[10px] uppercase tracking-[0.08em] text-faint">TVL</p>
          <p className="mt-1 truncate text-sm font-semibold tabular-nums text-fg">

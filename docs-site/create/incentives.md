...
**On-chain: the claim.** Accrued rewards do **not** arrive in an LP's wallet automatically as they provide liquidity. Eligible users **claim** their accumulated balance on-chain — through OpenPendle's **My positions** page or Merkl's own interface — which is when the reward tokens actually move. An address can let rewards build up and claim periodically; claiming is a normal signed transaction subject to gas on that chain.
...
- **It is claimed separately through Merkl's distributor.** Merkl rewards accrue off-chain and are claimed on-chain on the schedule Merkl sets. OpenPendle's **My positions** page can build that direct distributor call, or you can use Merkl's interface. OpenPendle never holds or distributes the reward tokens, and they do not land in the wallet automatically while trading. See [where an LP's return comes from](/guides/providing-liquidity#where-the-return-comes-from).
...
4. **Tells your LPs where to claim.** Liquidity providers can open **My positions** in OpenPendle to retrieve and claim Merkl rewards, or use Merkl's own interface. Campaign discovery and rules still live with Merkl.
...
OpenPendle operates no reward backend or indexer of its own. When a connected user opens **My positions**, the browser asks Merkl's public API for rewards on each supported chain. That request sends the **wallet address and chain ID** to Merkl and returns the cumulative amounts and Merkle proofs needed for a direct on-chain claim.

The page shows claimable Merkl rewards across chains and builds one transaction per chain against Merkl's distributor. OpenPendle does not compute campaign accounting, create campaigns, custody rewards, or add a fee. It also does not currently provide a campaign directory or calculate campaign APRs; Merkl remains the authoritative external source for campaign setup and rules. See [How OpenPendle works](/reference/architecture) for the full network-request disclosure.

## The limits, stated plainly

::: warning Merkl rewards are a variable bonus — and they cushion nothing
Merkl rewards on a community pool are **third-party-funded, off-chain-computed, not guaranteed, and can stop at any time** — many pools have none at all. They are claimed separately through Merkl's distributor, using either OpenPendle's **My positions** page or Merkl's interface; OpenPendle does not fund, custody, quote, or vouch for them. Crucially, no Merkl campaign changes the pool's underlying risk: there is **no gauge, no vePENDLE backing, and no guaranteed reward** to cushion a loss. If the unreviewed asset beneath the pool is malicious, broken, or simply fails, the [PT](/concepts/principal-tokens) may **not** redeem at par and an [LP](/concepts/liquidity-and-amm) position can lose value — a Merkl bonus does not offset that. **OpenPendle validates market provenance but cannot vouch for the assets or SY contracts underneath.** Community pools are permissionless and unreviewed — anyone can create one, and interacting with them can lose you funds. Experimental — use at your own risk. Not affiliated with Pendle Finance.
:::
...
| How do LPs receive rewards? | They **claim** on-chain through Merkl's distributor, using OpenPendle's **My positions** page or Merkl's interface. |
| What is the SY hook? | An `offchainRewardManager` set at SY creation enables `claimOffchainRewards`; `address(0)` disables it. |
| What does OpenPendle show today? | Claimable rewards across supported chains and a direct claim transaction; campaign setup and APR discovery remain on Merkl. |

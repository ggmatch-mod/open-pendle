...
None of those services sits between the wallet and an on-chain AMM transaction. Pendle's limit-order API is, however, part of the off-chain order path: it receives the signed order before takers can fill it through the Limit Router. If you want a copy that makes no ancillary third-party API calls, you can keep the same-origin factory snapshot while disabling Pendle enrichment, the ticker, PT/YT pool resolution, Yield alerts, limit orders, and Merkl rewards in your fork; core market-by-address reads and Pendle transactions can remain RPC-only.

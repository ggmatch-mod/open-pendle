...
| **Backend** | Conventional web stack | **No request-time OpenPendle application backend** — a scheduled job publishes a static catalog; no accounts or transaction relay; Cloudflare provides page-view/performance analytics |
...
The [Content-Security-Policy](/reference/architecture) reinforces this posture: it blocks JavaScript `eval()` and `Function`, permits WebAssembly used for cryptography, and allowlists Cloudflare's analytics script; fonts remain self-hosted. Core reads and transactions use the blockchain RPC you choose. Explore downloads the generated factory snapshot, which also supplies the primary PT/YT-to-pool mapping, and uses Pendle's API for optional listing enrichment. Other ancillary public calls go to DefiLlama/CoinGecko for the ticker, Pendle/Blockscout for live lookup fallbacks, Merkl on **My positions**, and Cloudflare Web Analytics for page-view and performance metrics. None is in the transaction-signing path.
...
- **A privacy-conscious user.** You want an interface with no OpenPendle accounts or user-data backend, where saved pools and RPC settings stay in your browser. You also understand the Cloudflare page-view/performance analytics and direct RPC and ancillary public-service requests disclosed in [How OpenPendle works](/reference/architecture), including Merkl receiving the wallet address and chain ID on **My positions**.

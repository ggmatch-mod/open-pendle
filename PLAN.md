...
- [x] **Per-chain address book:** turn `addresses.ts` into a `chainId → AddressBook` map (router, routerStatic, pyYtLpOracle, commonDeploy, syFactory, all factory generations + active pair, treasury, multicall). Sourced from Pendle's `deployments/<id>-core.json`, verified live at runtime (F12 still applies per chain). Router V4 is the same vanity address cross-chain; factory *generations* differ per chain. → `ADDRESS_BOOKS: Record<SupportedChainId, AddressBook>`, `addressBookFor(chainIdOrClient)`, `chainIdOf`.
- [x] **Chain threading:** every lib fn already takes a `PublicClient` — add an explicit `chainId`/`AddressBook` param (or resolve from the client's chain) so no address is hardcoded to Arbitrum; validate markets against *that chain's* factory set. → `addressBookFor(client)` resolved inside each lib fn (no signature changes).
- [x] **wagmi config:** register all supported chains; per-chain RPC (user-configurable via `openpendle.rpc.<chainId>`); sensible default public RPC per chain.
- [x] **Network dropdown** in the header: switches the active chain (`useSwitchChain`); loader/pool page operate on the selected chain; the wrong-network banner generalizes to "switch to <selected chain>". → `NetworkSelector.tsx` + `useActiveChain()` (localStorage `openpendle.chain`, cross-tab synced).
- [x] **Registry keyed by (chainId, market):** `SavedPool.chainId` already exists (currently hardcoded `42161`) — generalize it; storage stays `openpendle.pools.v1` (chainId is already in the shape, so no schema bump). **Home "Your pools" shows ALL remembered pools grouped + badged by network** (not filtered to the active chain), so switching networks and returning always shows a chain's pools; a pool on a non-active chain shows a one-click "switch to <chain>" affordance. *(This satisfies the user's requirement and is strictly more useful than filtering.)* → sweep results keyed by `sweepKey(chainId, market)`.
- [x] **Per-chain capability degradation:** chains without `commonDeploy` → hide the M6 create wizard there; chains without RouterStatic → quoter falls back to the simulation-only default-ApproxParams path (M0); per-chain factory-generation counts drive the validation set.
- [x] **Fork-test** the core read + trade (+ create where supported) flow on each shipped chain (a fork per chain). → m8-multichain-test 19/19 (Base + Ethereum); Arbitrum regressions m2–m7 all pass.
...
### v1.5 roadmap (post-launch, user-directed)

Deferred-but-planned features, recorded as they were decided:

1. **Aggregator zaps from arbitrary tokens** (user-approved 2026-07-04) — client-side KyberSwap/ODOS calldata via the router's `SwapData`/`PendleSwap`, so a user can zap in/out with *any* token, not just SY-accepted ones. Third-party aggregator API, not our backend.
2. **Protocol-added incentives via Merkl** (user-directed 2026-07-04) — community pools can't use Pendle's native gauges, so incentivizing an OpenPendle pool goes through Merkl (Angle). v1.0 already ships the deep-link nudge under the Remember toggle → studio.merkl.xyz; v1.5 surfaces active Merkl campaigns on the pool page + an on-chain claim button. Zero custom contracts.
3. **Third-party embedding** (user question 2026-07-04 — confirmed sensible/possible) — let other sites integrate a specific community pool's OpenPendle UI. Natural fit for the static, backend-free, wallet-signed architecture (how Uniswap/0x/LI.FI widgets work). Three depths, shipped in order: (a) **iframe embed mode** (`?embed=1&theme=…` renders one pool's panels, no app shell — fastest, universal); (b) **headless SDK** (`@openpendle/sdk` — publish the framework-free `lib/` readers/quoters/plan-builders); (c) **React widget** (`@openpendle/widget` — themeable component sharing the host's wallet, the cleanest native embed; avoids the WalletConnect-in-iframe friction of (a)). Security is already covered: any embed can only pass a pool address, which OpenPendle validates against Pendle's factories regardless of load path.
